...
class Dn3mf1:
    def __init__(self, nme, x, dc, factor, red_dim):
        ...
        for fac in self.factor:
            ...
            fn2 = self.nme + "/" +self. nme + "_an3mfg_w_" + str(dc) + "_" + str(self.red_dim[self.l - 1]) + "_" + str(fac) + ".txt"
            f2 = open(fn2, "r")
            self.dct["w{0}".format(self.l)] = np.genfromtxt(f2, delimiter = ',')
            f2.close()
            w_r, w_c = self.dct["w{0}".format(self.l)].shape
            self.dct["momentum_w{0}".format(self.l)] = np.zeros((w_r, w_c))
            self.dct["x{0}hat".format(self.l-1)] = np.zeros((self.m, w_c))
        ...
        self.J2 = 0.0
        self.costarray = np.array([0, 0, 0, 0])
        print("Dn3mf1 init ends")
    
    def forward_propagation(self):
        print("Dn3mf1 forward propagation begins")
        ...
        for itr in range(self.l):
            if (itr == 0):
                self.dct["x{0}hat".format(self.l-itr-1)] = np.matmul(self.dct["x{0}".format(self.l-itr)], self.dct["w{0}".format(self.l-itr)])
            else:
                self.dct["x{0}hat".format(self.l-itr-1)] = np.matmul(self.dct["x{0}hat".format(self.l-itr)], self.dct["w{0}".format(self.l-itr)])
            self.dct["x{0}hat".format(self.l-itr-1)] = General().sigmoid1(self.dct["x{0}hat".format(self.l-itr-1)])
        print("Dn3mf1 forward propagation ends")
    
    def cost(self):
        print("Dn3mf1 cost begins")
        self.J1 = (1 / (2 * self.m * self.n)) * ((np.square(self.dct["x{0}".format(0)] - self.dct["x{0}hat".format(0)])).sum())
        self.J2 = (self.lambdaa / (2 * self.n * self.n)) * ((np.square(self.vwi)).sum())
        self.J = self.J1 + self.J2
        print("Dn3mf1 cost ends")
    
    def bp_vw(self, v_s, v_e, w_s, w_e):
        if (v_e == 0):
            for itr in range(w_s, w_e+1):
                if (itr == w_s):
                    w = self.dct["w{0}".format(self.l-itr+1)]
                else:
                    w = np.matmul(w, self.dct["w{0}".format(self.l-itr+1)])
            bpvw = w
        elif (w_e == 0):
    ...
    def backward_propagation(self):
        print("Dn3mf1 backward propagation begins")
        dct1 = {}
        for itr in range(self.l):
            q1 = itr + 1
            if (q1 == 1):
                theta = ((self.dct["x{0}".format(0)] - self.dct["x{0}hat".format(0)]) * self.dct["x{0}hat".format(0)] * (1 - self.dct["x{0}hat".format(0)]))
                w_a = (-1 / (self.m * self.n)) * (np.matmul(self.dct["x{0}hat".format(q1)].T, theta))
                bpvw = self.bp_vw(1, self.l, 1, self.l-q1)
                w_b = (self.lambdaa / (self.n * self.n)) * (np.matmul(bpvw.T, self.vwi))
            elif (q1 == self.l):
                theta = (np.matmul(theta, self.dct["w{0}".format(q1-1)].T)) * self.dct["x{0}hat".format(q1-1)] * (1 - self.dct["x{0}hat".format(q1-1)])
                w_a = (-1 / (self.m * self.n)) * (np.matmul(self.dct["x{0}".format(q1)].T, theta))
                bpvw1 = self.bp_vw(1, self.l, 0, 0)
                bpvw2 = self.bp_vw(0, 0, self.l-q1+2, self.l)
                w_b = (self.lambdaa / (self.n * self.n)) * np.matmul((np.matmul(bpvw1.T, self.vwi)), bpvw2.T)
            else:
                theta = (np.matmul(theta, self.dct["w{0}".format(q1-1)].T)) * self.dct["x{0}hat".format(q1-1)] * (1 - self.dct["x{0}hat".format(q1-1)])
                w_a = (-1 / (self.m * self.n)) * (np.matmul(self.dct["x{0}hat".format(q1)].T, theta))
                bpvw1 = self.bp_vw(1, self.l, 1, self.l-q1)
                bpvw2 = self.bp_vw(0, 0, self.l-q1+2, self.l)
                w_b = (self.lambdaa / (self.n * self.n)) * np.matmul((np.matmul(bpvw1.T, self.vwi)), bpvw2.T)
        ...
        for itr in range(self.l, 0, -1):
            q2 = itr
            if (q2 == self.l):
                theta = (np.matmul(theta, self.dct["w{0}".format(self.l)].T)) * self.dct["x{0}".format(self.l)] * (1 - self.dct["x{0}".format(self.l)])
                v_a = (-1 / (self.m * self.n)) * (np.matmul(self.dct["x{0}".format(q2-1)].T, theta))
                bpvw1 = self.bp_vw(1, q2-1, 0, 0)
                bpvw2 = self.bp_vw(0, 0, 1, self.l)
                ...
                v_a = (-1 / (self.m * self.n)) * (np.matmul(self.dct["x{0}".format(q2-1)].T, theta))
                bpvw = self.bp_vw(q2+1, self.l, 1, self.l)
                v_b = (self.lambdaa / (self.n * self.n)) * (np.matmul(self.vwi, bpvw.T))
            else:
                theta = (np.matmul(theta, self.dct["v{0}".format(q2+1)].T)) * self.dct["x{0}".format(q2)] * (1 - self.dct["x{0}".format(q2)])
                v_a = (-1 / (self.m * self.n)) * (np.matmul(self.dct["x{0}".format(q2-1)].T, theta))
                bpvw1 = self.bp_vw(1, q2-1, 0, 0)
                bpvw2 = self.bp_vw(q2+1, self.l, 1, self.l)
                v_b = (self.lambdaa / (self.n * self.n)) * np.matmul((np.matmul(bpvw1.T, self.vwi)), bpvw2.T)
        ...
        for itr in range(self.l):
            self.dct["w{0}".format(itr + 1)] = dct1["w{0}_u".format(itr + 1)]
            self.dct["v{0}".format(itr + 1)] = dct1["v{0}_u".format(itr + 1)]
        print("Dn3mf1 backward propagation ends")
    
    def model(self):
        print("Dn3mf1 model begins")
        while self.step < 1:#20
            while self.step < 5:#self.J > 1 or abs(self.J_old - self.J) > 0.0000001:#
                self.step = self.step + 1
                self.J_old = self.J
                ...
                w = np.identity(v_c)
                for itr in range(self.l):
                    w = np.matmul(w, self.dct["w{0}".format(self.l-itr)])
                self.vw = np.matmul(v, w)
                self.vwi = self.vw - self.I
                self.cost()
                self.backward_propagation()
                self.costarray = np.append(self.costarray, [self.step, self.J1, self.J2, self.J])
                print("Dn3mf1 step:", self.step, "J1:", np.around(self.J1, decimals = 10), "J2:", np.around(self.J2, decimals = 10), "J:", np.around(self.J, decimals = 10))
            fname1 = self.nme + "/" + self.nme + "_dn3mf1_b_" + str(self.factor[self.l - 1]) + ".txt"
            f1 = open(fname1, "w")
            np.savetxt(f1, self.dct["x{0}".format(self.l)], fmt = '%.17f', delimiter = ',')
            f1.close()
            costmatrix = self.costarray.reshape(int(self.costarray.size/4), 4)
            costmatrix = np.delete(costmatrix, (0), axis = 0)
            fname2 = self.nme + "/" + self.nme + "_dn3mf1_costmatrix_"+ str(self.factor[self.l - 1]) + ".txt"
            f2 = open(fname2, "w")
            ...
            plt.title(titl) 
            plt.plot(costmatrix[:, 3:])
            imgname = self.nme + "_dn3mf1_" + str(self.m) + "_" + str(self.n) + "_" + str(self.factor[self.l - 1]) + ".png"
            plt.savefig(self.nme+"/"+imgname)
            plt.clf()
            fname3 = self.nme + "/" + self.nme + "_dn3mf1_size_cost_" + str(self.factor[self.l - 1]) + ".txt"
            f3 = open(fname3, "w")
            f3.write("%s, %s, %s, %s, %s, %s\n" % (self.m, self.n, self.step, self.J1, self.J2, self.J))
            f3.close()
        print("Dn3mf1 model ends")
...
def main(data_choice, f):
    ...
    obj4 = Dn3mf1(nme, x, dc, factor, red_dim)
    obj4.model()

if __name__ == '__main__':
    data_choice = 1 # 1: GastrointestinalLesionsInRegularColonoscopy, 2: OnlineNewsPopularity, 3: ParkinsonsDiseaseClassification
    f = 0.25
    main(data_choice, f)
